...
export default function JobFlowPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    supabase
      .from('jobs')
      .select('type, vehicles(make, model, year, plate)')
      .eq('id', id)
      .single()
      .then(({ data }) => {
        if (data) {
          setJob(data as unknown as JobData)
          // Restore done sections from sessionStorage if available
          const saved = sessionStorage.getItem(`job_flow_${id}_done`)
          if (saved) {
            try { setDoneSections(new Set(JSON.parse(saved))) } catch { /* ignore */ }
          }
        }
        setLoading(false)
      })
